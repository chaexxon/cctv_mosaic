...
def _register_face(face_path: Path, name: str) -> None:
...
async def process_video_web(
    ...
    for idx, face_image in enumerate(face_images):
        ...
        face_path = UPLOAD_DIR / f"{uid}_face_{idx}.jpg"
        ...
        try:
            ...
            print(f"[face-register] failed: {e}")

    # 4. 번호판 등록
    if plate_number.strip():
        _register_plate(plate_number, owner=f"user_{uid}")
